...
quake_map <- recent_quakes |> 
  ...
  addCircleMarkers(
    ...
    label = ~ paste(
      ...
      "マグニチュード:", round(magnitude, 1), "<br/>", 
      "深さ:",  round(depth), " km"
      ) |> map(html),
    labelOptions = c(textsize = "15px")) |> 
  addLegend(title = "マグニチュード", colors = mag_pal(0:5), labels = c("<1", 1:4,">5")) |> 
...
top_n_table <- top_n |> 
  ...
  tab_source_note(
    source_note = md(paste("[GeoNet API](https://api.geonet.org.nz/)から取得した日時:", format(now_nz, "%Y年%m月%d日 %H:%M %Z")))
  )
```
